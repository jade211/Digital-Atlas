...
2. [System Architecture](#2-system-architecture)
   - [High Level Overview](#21-high-level-overview)
     - [Backend](#211-backend)
     - [Frontend](#212-frontend)
   - [Distribution of Functions](#22-distribution-of-functions)
   - [Reused or 3rd Party Components](#23-reused-or-3rd-party-components)
3. [High-Level Design](#3-high-level-design)
   - [Data Flow Diagram](#31-data-flow-diagram)
   - [Object Model](#32-object-model)
4. [Problems and Resolution](#4-problems-and-resolution)
5. [Installation Guide](#5-installation-guide)
   - [Prerequisites and Required Software and Hardware Requirements](#51-prerequisites-and-required-software-and-hardware-requirements)
   - [Step-By-Step Guide](#52-step-by-step-guide)
   - [Dependencies List and Version](#53-dependencies-list-and-version)
...
2. **Data Retrieval:** The SearchArea component fetches the data modules for crime, transport, housing, amenities, and schools. Alternatively, if the user is accessing data from the navbar in the individual feature components, they will input their specific area and receive information pertaining to that particular category of interest in that area.
...
Databases: Our backend has utilized Django to store information relevant to statistics such as the transport information and housing prices. The databases we have taken this information from include:
...
Overall Overview: A user will input their area of interest, whether it be a town or county in Ireland. The frontend components will then be triggered and will process the user's search term and search option - which will send the fetch query to backend. The backend processes then queries and fetches the data from the relevant APIs and datasets and prepares the response. Processed data is then presented on the frontend web pages - showcasing the fetched statistics for that area. Users can then leave comments on that particular area.

### 2.2 Distribution of Functions
...
Data Retrieval: The SearchArea component dynamically calls specialized modules for crime, transport, housing, amenities, and schools to fetch relevant data specific to the entered location.
...
### 2.3 Reused or 3rd Party Components:

#### Geocode API - Geoapify API:

Purpose: This API is utilized for geocoding purposes, converting location-based information (like addresses or place names) into geographic coordinates (latitude and longitude) and Place_ID numbers.
...
- **Data Processing and API Interaction**: Manages user queries received from the frontend search bars by using fetch requests to get the data from the APIs. The API organizations used include the CSO, Geoapify, and Mapbox.
...
- **Frontend to Backend**: 
  ...
  - Example: The Transport Component Page
    ...
    - Both of these datasets are returned and presented on the frontend component.
...
- Install Node.js and npm: Download and install Node.js from [here](https://nodejs.org/). This will also install npm.
- Install Python and pip: Download and install Python from [here](https://www.python.org/). Make sure to check the box that says "Add Python to PATH" during installation.
